...
panel <- expand_grid(city = 1:n_cities, t = 1:n_t) |>
  ...
  mutate(
    ...
    eff = if_else(treated, 0.6 * (1 + 0.05 * (t - g)) *
                            (1 + 0.3 * (g == 8) - 0.2 * (g == 16)), 0),
...
<a href="#staggered-dgp-detail" class="nav-btn">DGP code</a>
...
```{r bacon-viz, echo=FALSE, fig.height=4}
# Hand-computed Bacon decomposition for our 4-cohort DGP
panel_long <- panel |> mutate(D = as.integer(treated))
# 2x2 DiDs: (treated cohort, control cohort, weight, estimate)
get_2x2 <- function(g_t, g_c, df, t_max) {
  ...
  treated_pre  <- mean(df$y[df$g == g_t & df$t %in% pre])
  treated_post <- mean(df$y[df$g == g_t & df$t %in% post])
  control_pre  <- mean(df$y[df$g == g_c & df$t %in% pre])
  control_post <- mean(df$y[df$g == g_c & df$t %in% post])
  (treated_post - treated_pre) - (control_post - control_pre)
}
bacon <- expand_grid(g_t = c(8, 12, 16), g_c = c(8, 12, 16, Inf)) |>
  filter(g_t != g_c) |>
  rowwise() |>
  mutate(est = get_2x2(g_t, g_c, panel_long, n_t)) |>
  ungroup() |>
  mutate(comparison = case_when(
    is.infinite(g_c) ~ "Treated vs Never",
    g_c > g_t ~ "Earlier vs Later (later as control: clean)",
    TRUE ~ "Later vs Earlier (earlier as control: PROBLEM)"
  ))
ggplot(bacon, aes(paste0("g=", g_t, " vs g=", ifelse(is.infinite(g_c), "∞", g_c)),
                  est, fill = comparison)) +
  geom_col() + coord_flip() +
  geom_hline(yintercept = 0) +
  scale_fill_manual(values = c("Treated vs Never" = "#4daf4a",
                                "Earlier vs Later (later as control: clean)" = "#377eb8",
                                "Later vs Earlier (earlier as control: PROBLEM)" = "#e41a1c")) +
  labs(x = NULL, y = "2×2 DiD estimate", fill = NULL) +
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
```

Red bars: when an *already-treated* cohort serves as the comparison, you difference out a treated trend. TWFE puts weight on these.
...
|---|---|---|---|
| **Callaway & Sant'Anna** | 2021 | Group-time ATT $(g, t)$, then aggregate | `did` |
...
| **Borusyak, Jaravel, Spiess** | 2024 | Impute $Y(0)$ from never-treated, then average residuals | `didimputation` |
]
...
- **Smoothness ($M$):** the post-treatment violation can be at most $M$ times larger than the largest pre-treatment violation.
...
sc_long <- bind_rows(
  tibble(t = 1:n_T, y = treated_y, unit = "Treated"),
  expand_grid(t = 1:n_T, j = 1:n_donor) |>
    mutate(y = donors[cbind(t, j)], unit = paste0("Donor ", j))
)

ggplot(sc_long, aes(t, y, group = unit,
                    color = unit == "Treated", linewidth = unit == "Treated")) +
  geom_line(alpha = 0.7) +
...
Choose weights $w_j \ge 0$ with $\sum_j w_j = 1$ to match the treated unit's *pre-treatment* outcomes:

$$\min_{w} \sum_{t < T_0} \left( Y_{1t} - \sum_{j \ge 2} w_j Y_{jt} \right)^2 \;\; \text{s.t.}\;\; w_j \ge 0,\;\; \sum_j w_j = 1$$

.small[
```{r sc-fit, fig.height=2.8}
# NNLS-style projection on the simplex (simplified: quadprog or by hand)
pre <- 1:(t_treat - 1)
A <- donors[pre, ]; b <- treated_y[pre]
# Project to simplex via solve.QP (quadratic prog)
library(quadprog)
Dmat <- 2 * t(A) %*% A; dvec <- 2 * t(A) %*% b
Amat <- cbind(rep(1, n_donor), diag(n_donor))      # sum=1, w_j>=0
...
The fitted weights match true weights closely on donors 1-3.
...
.blue-box[
- **Unit weights** ($\hat\omega_i$) — like SC, match pre-treatment trajectories of donors to the treated unit.
- **Time weights** ($\hat\lambda_t$) — match pre-treatment outcomes of the treated unit to its own post-treatment level (down-weights periods that don't look like "now").
]
...
.blue-box[
- **`grf::policy_tree`** — fit a depth-$L$ decision tree over $X$ that maximizes welfare. Output: an interpretable rule like *"deploy in dense cities to drivers with <2 years tenure."*
- **Athey & Wager (2021):** regret bounds for these rules.
]
...
**Thompson Sampling** (the workhorse): maintain a posterior over each arm's reward, sample from it, play the argmax. Asymptotically optimal regret; minimal tuning.
]
...
.small[
**Problem:** classical $p$-values are valid only at one fixed $n$. Peeking inflates Type-I error — by $t=N$, the running $p$-value will dip below 0.05 about 30% of the time *under the null*.
...
.small[
Under $H_0$, naive peeking yields ≈30% rejection rate, not 5%. mSPRT or OBF would hold this near 5%.
]
...
**Want effect heterogeneity?** → **Causal forest** for $\hat\tau(x)$; **policy tree** for an interpretable deployment rule.
...
.blue-box[
**"How do you get heterogeneous treatment effects?"** Causal forest from `grf`. Honest splitting prevents over-fit. Aggregate to ATE for sanity-check; predict $\hat\tau(x)$ for new units; feed into a policy tree if a deployment rule is needed.
]
...
The standard pretrend test asks: do the pre-treatment leads have $p > 0.05$? Failing to reject is taken as evidence parallel trends holds.

**The problem:** these tests have low power against linear (or near-linear) violations — exactly the kind that would bias the post-treatment estimate.

```{r roth-power, echo=FALSE, fig.height=3.2}
# Simulate: under a linear violation, what's the prob of detecting it?
...
Even a *meaningful* linear violation is missed most of the time. Roth's recommendation: use Honest DiD bounds rather than relying on the pretrend test.
...
<a href="#staggered-adoption-what-changes" class="nav-btn">← back</a>
...
<a href="#the-twfe-estimator-looks-innocent-isnt" class="nav-btn">← back</a>
...
<a href="#synthetic-did-the-bridge" class="nav-btn">← back</a>
...
<a href="#callaway-santanna-in-practice" class="nav-btn">← back</a>
...
<a href="#causal-forest-in-practice-grf" class="nav-btn">← back</a>
...
<a href="#causal-forest-how-it-works" class="nav-btn">← back</a>
...
<a href="#sequential-testing-in-one-slide" class="nav-btn">← back</a>
