---
title: "Module 1: The Experimental Ideal"
subtitle: "Potential Outcomes, Selection Bias, and Why Randomization Works"
output:
  xaringan::moon_reader:
    css: [default, default-fonts]
    nature:
      highlightStyle: github
      highlightLines: true
      countIncrementalSlides: false
      ratio: "16:9"
---

```{css, echo=FALSE}
.remark-code, .remark-inline-code { font-size: 16px; }
.remark-slide-content { padding: 1em 2em; font-size: 21px; }
.small { font-size: 80%; }
.tiny { font-size: 65%; }
.highlight-box {
  background: #fff3e0;
  border-left: 4px solid #e65100;
  padding: 0.5em 1em;
  margin: 0.5em 0;
}
.blue-box {
  background: #e3f2fd;
  border-left: 4px solid #1565c0;
  padding: 0.5em 1em;
  margin: 0.5em 0;
}
.btn-link {
  display: inline-block;
  padding: 0.15em 0.7em;
  background: #1565c0;
  color: white !important;
  text-decoration: none;
  border-radius: 4px;
  font-size: 0.75em;
}
.btn-link:hover { background: #0d47a1; }
.nav-btn-br {
  position: absolute;
  bottom: 12px;
  right: 70px;
  font-size: 11px;
  background: #e8eaf6;
  padding: 2px 8px;
  border-radius: 3px;
  z-index: 100;
  text-decoration: none;
  color: #1a237e;
}
.nav-btn-br:hover { background: #c5cae9; }
.tight-slide { font-size: 18px !important; }
```

```{r setup, include=FALSE}
knitr::opts_chunk$set(
  echo = TRUE, fig.align = "center",
  fig.width = 10, fig.height = 4.2, dpi = 150,
  message = FALSE, warning = FALSE
)
library(tidyverse)
library(estimatr)
theme_set(theme_minimal(base_size = 14))
set.seed(42)
```

# Course Map

<table>
<tr><th>#</th><th>Module</th><th>Status</th></tr>
<tr><td><b>1</b></td><td><b>The Experimental Ideal</b> <i>(you are here)</i></td><td>current</td></tr>
<tr><td>2</td><td><a href="../module-02/slides.html">SUTVA and When It Breaks</a></td><td>✓ done</td></tr>
<tr><td>3</td><td><a href="../module-03/slides.html">Designing Around Interference</a></td><td>✓ done</td></tr>
<tr><td>4</td><td><a href="../module-04/slides.html">Power and Sample Size</a></td><td>✓ done</td></tr>
<tr><td>5</td><td><a href="../module-05/slides.html">Analyzing Experiments</a></td><td>✓ done</td></tr>
<tr><td>6</td><td><a href="../module-06/slides.html">Multiple Testing & Subgroups</a></td><td>✓ done</td></tr>
<tr><td>7</td><td><a href="../module-07/slides.html">External Validity</a></td><td>✓ done</td></tr>
<tr><td>8</td><td><a href="../module-08/slides.html">Beyond the A/B Test</a></td><td>✓ done</td></tr>
</table>

---

# The Setup: A Zone-Notification Experiment

When a driver is heading into a zone with higher-than-average subsequent demand, a ride-sharing platform wants to know: **does sending the driver a push notification affect their decision to accept the next ride offer — and their earnings?**

--

The product team looks at the data:

```{r naive-data, echo=FALSE}
n <- 1000
drivers <- tibble(
  experience = rnorm(n),
  # More experienced drivers are more likely to have notifications enabled
  # LPM for selection: P(D=1) = 0.5 + 0.15 * experience, clipped to [0,1]
  got_notification = rbinom(n, 1,
    prob = pmin(1, pmax(0, 0.5 + 0.15 * experience))),
  # LPM for outcome: P(Y=1) = 0.4 + 0.2*experience + 0.05*D, clipped to [0,1]
  # True effect of notification on accept rate = 0.05 (5pp)
  accepted = rbinom(n, 1,
    prob = pmin(1, pmax(0, 0.4 + 0.2 * experience + 0.05 * got_notification)))
)

drivers |>
  group_by(got_notification) |>
  summarise(n = n(), accept_rate = mean(accepted)) |>
  mutate(got_notification = if_else(got_notification == 1,
                                     "Notified", "Not notified")) |>
  knitr::kable(col.names = c("Group", "N", "Accept rate"), digits = 3)
```

--

"The notification increases accept rate by ~15 percentage points! Ship it!"

...or does it?

---

# The Same Problem at an Online Retailer

- A large online retailer offers a paid membership program (annual fee, free fast shipping plus perks); the core question is whether membership causes incremental spend or whether heavy shoppers self-select in.
- The naive comparison (member vs non-member average spend) is exactly the selection-bias decomposition: naive difference = ATT + selection term.
- The selection term is positive: heavy-frequency buyers join disproportionately and would have spent more even without membership.
- ATE and ATT diverge here, because the customers most likely to join (and who drive ATT) are those who already buy frequently, not the average customer.
- Randomizing membership directly is commercially infeasible; a randomized signup discount is the feasible design, foreshadowing the encouragement framework in Module 5.

---
name: fundamental-problem

# The Fundamental Problem

We want to know the **causal effect** of the notification on each driver $i$'s accept decision:

$$\tau_i = Y_i(1) - Y_i(0)$$

--

| Driver | $Y_i(1)$ (notified) | $Y_i(0)$ (not notified) | Effect |
|--------|---------------------|--------------------------|--------|
| Alice  | 1 (accepted)        | ?                        | ?      |
| Bob    | ?                   | 0 (declined)             | ?      |
| Carol  | 1 (accepted)        | ?                        | ?      |
| Dave   | ?                   | 1 (accepted)             | ?      |

--

We only observe **one** potential outcome per driver. The other is the *counterfactual* — it doesn't exist in the data. We can never compute $\tau_i$ directly.

---
name: average-effects

# What We *Can* Estimate: Average Effects

Even though individual effects are unobservable, we can estimate **averages**:

--

- **ATE** = $E[Y_i(1) - Y_i(0)]$ — average effect across *everyone*

--

- **ATT** = $E[Y_i(1) - Y_i(0) \mid D_i = 1]$ — average effect on those who *got* treatment

--

- **ATU** = $E[Y_i(1) - Y_i(0) \mid D_i = 0]$ — average effect on the *untreated*

--

.highlight-box[
**When do these differ?** When (1) treatment effects are heterogeneous AND (2) who gets treated is correlated with the effect. Example: if experienced drivers both opt into notifications AND benefit more from them, then ATT > ATE > ATU.
]

---
name: ignorability-main

# Ignorability: The Assumption That Substitutes for Randomization

When we *can't* randomize, what assumption substitutes for it?

--

.pull-left[
**Ignorability** (a.k.a. **unconfoundedness**, **CIA**, **selection on observables**):

$$\big( Y(0),\, Y(1) \big) \;\perp\!\!\!\perp\; D \,\big|\, X$$

Treatment assignment is independent of potential outcomes, conditional on $X$.

**With overlap** ( $0 < P(D=1\mid X) < 1$ on the support of $X$ ), this is enough to identify the ATE:

$$\tau \;=\; \mathbb{E}_X\!\big[\, \mathbb{E}[Y \mid D=1, X] - \mathbb{E}[Y \mid D=0, X] \,\big]$$
]

.pull-right[
.small[
**Three regimes:**

- **Randomization:** $D$ is independent of *everything* by design ⇒ ignorability holds with $X = \emptyset$. Selection bias is mechanically zero.
- **Selection on observables:** ignorability holds *given $X$*. Matching, regression adjustment, IPW all rest on this — and are only as good as $X$.
- **Selection on unobservables:** confounders are missing from $X$ ⇒ ignorability fails ⇒ ATE is not point-identified. Reach for IV, RDD, DiD, or sensitivity bounds.
]
]

--

.small[
**Why this matters here.** The selection-bias decomposition on the next slide shows what *fails* when ignorability fails. The notification example violates it — driver experience determines both $D$ and $Y(0)$ — and we don't observe experience perfectly.
]

<a href="#ignorability-formal" class="btn-link">formal statement →</a>

---
name: selection-decomp

# Selection Bias: The Decomposition

The naive comparison — difference in observed means — is:

$$E[Y_i \mid D_i = 1] - E[Y_i \mid D_i = 0]$$

--

This equals:

$$= \underbrace{E[Y_i(1) - Y_i(0) \mid D_i = 1]}_{\text{ATT}} + \underbrace{E[Y_i(0) \mid D_i = 1] - E[Y_i(0) \mid D_i = 0]}_{\text{selection bias}}$$

--

.highlight-box[
**Selection bias** = the difference in *baseline* outcomes between the groups. Drivers who opted into notifications are more experienced → they would have accepted more rides *even without the notification*.
]

<a href="#selection-proof" class="btn-link">see proof →</a>

---

# Selection Bias: Visualized

```{r selection-bias-viz, echo=FALSE, fig.height=4}
drivers |>
  mutate(Group = if_else(got_notification == 1,
                          "Notified", "Not notified")) |>
  ggplot(aes(experience, fill = Group)) +
  geom_density(alpha = 0.5) +
  geom_vline(data = drivers |>
               group_by(got_notification) |>
               summarise(m = mean(experience)) |>
               mutate(Group = if_else(got_notification == 1,
                                       "Notified", "Not notified")),
             aes(xintercept = m, color = Group),
             linetype = "dashed", linewidth = 1) +
  labs(title = "The groups are NOT comparable",
       subtitle = "Drivers who got notifications have higher baseline experience",
       x = "Experience (unobserved)", y = "Density") +
  theme(legend.position = "top")
```

The notified group has higher experience on average → higher accept rate *regardless of the notification*. The naive estimate conflates the notification effect with pre-existing differences.

---

# Randomization: The Fix

Randomly assign 500 drivers to receive the notification, 500 to control:

```{r randomize}
set.seed(132)
n <- 1000
rct <- tibble(
  driver_id = 1:n,
  experience = rnorm(n),
  # Random assignment — independent of experience
  notification = sample(rep(c(0, 1), each = n/2)),
  # LPM outcome: P(accept) = 0.4 + 0.2*experience + 0.05*notification (clipped)
  # True effect of notification = 0.05 (5pp)
  accepted = rbinom(n, 1,
    prob = pmin(1, pmax(0, 0.4 + 0.2 * experience + 0.05 * notification)))
)
```

--

```{r rct-balance, echo=FALSE}
rct |>
  group_by(notification) |>
  summarise(n = n(), mean_experience = mean(experience),
            accept_rate = mean(accepted)) |>
  mutate(notification = if_else(notification == 1, "Notified", "Control")) |>
  knitr::kable(digits = 3, col.names = c("Group", "N", "Mean experience",
                                           "Accept rate"))
```

Experience is balanced (by construction). The difference in accept rate is an unbiased estimate of the ATE.

---
name: why-randomization-works

# Why Does Randomization Work?

Random assignment makes treatment independent of potential outcomes:

$$(Y_i(1), Y_i(0)) \perp D_i$$

--

This kills selection bias:

$$E[Y_i(0) \mid D_i = 1] = E[Y_i(0) \mid D_i = 0]$$

--

So the simple difference in means equals the ATE:

$$E[Y \mid D=1] - E[Y \mid D=0] = E[Y(1)] - E[Y(0)] = \text{ATE}$$

--

.blue-box[
**But:** randomization guarantees this *in expectation*, not in every sample. Any given experiment can have imbalanced covariates by chance. That's what inference (confidence intervals, p-values) is for.
]

---
name: randomization-sim

# Simulation: Randomization Works in Expectation

```{r sim-many-rcts, echo=FALSE, fig.height=3.8}
# Under LPM the true ATE is (approximately) the coefficient on D
true_ate <- 0.05

sim_rct <- function(i) {
  notification <- sample(rep(c(0, 1), each = n/2))
  prob_y <- pmin(1, pmax(0, 0.4 + 0.2 * rct$experience + 0.05 * notification))
  accepted <- rbinom(n, 1, prob = prob_y)
  mean(accepted[notification == 1]) - mean(accepted[notification == 0])
}

estimates <- tibble(
  estimate = map_dbl(1:500, sim_rct)
)

ggplot(estimates, aes(estimate)) +
  geom_histogram(bins = 40, fill = "steelblue", alpha = 0.7) +
  geom_vline(xintercept = true_ate, color = "firebrick",
             linewidth = 1.2, linetype = "dashed") +
  annotate("text", x = true_ate + 0.005, y = 60,
           label = paste("True ATE =", round(true_ate, 3)),
           color = "firebrick", hjust = 0, size = 4) +
  labs(title = "500 simulated RCTs: estimates centered on the truth",
       subtitle = "Each bar is one experiment. Variation = sampling noise, not bias.",
       x = "Estimated ATE (difference in accept rates)", y = "Count")
```

.small[
**What sets the spread?** The standard error of the difference in means, $\text{SE} = \sqrt{\sigma_1^2/n_1 + \sigma_0^2/n_0}$. Larger $n$ → narrower; less outcome variance ( $\sigma^2$ ) → narrower. Pre-treatment covariates or stratification can tighten it further (Module 5).
]

---

# The Naive Estimate Is Biased

**Naive** = *observational*: no randomization. Drivers self-select into notifications — more experienced drivers opt in with higher probability, $P(D=1) = 0.5 + 0.15 \cdot \text{experience}$ (clipped to $[0,1]$). We then compute the simple difference in accept rates, as if it were an RCT.

```{r sim-naive, echo=FALSE, fig.height=3.8}
sim_naive <- function(i) {
  # Non-random: LPM for self-selection — more experience → higher P(notif on)
  prob_notif <- pmin(1, pmax(0, 0.5 + 0.15 * rct$experience))
  notification <- rbinom(n, 1, prob = prob_notif)
  prob_y <- pmin(1, pmax(0, 0.4 + 0.2 * rct$experience + 0.05 * notification))
  accepted <- rbinom(n, 1, prob = prob_y)
  mean(accepted[notification == 1]) - mean(accepted[notification == 0])
}

naive_estimates <- tibble(estimate = map_dbl(1:500, sim_naive))

bind_rows(
  estimates |> mutate(method = "Randomized"),
  naive_estimates |> mutate(method = "Observational (naive)")
) |>
  ggplot(aes(estimate, fill = method)) +
  geom_histogram(bins = 40, alpha = 0.6, position = "identity") +
  geom_vline(xintercept = true_ate, color = "firebrick",
             linewidth = 1.2, linetype = "dashed") +
  labs(title = "Randomized vs Naive: bias is visible",
       subtitle = "Naive estimates are systematically too high (selection bias)",
       x = "Estimated effect", y = "Count") +
  theme(legend.position = "top")
```

The naive distribution is shifted right — it **overestimates** the effect because experienced drivers both opt into notifications and accept more.

---

# Numerical Decomposition

.pull-left[
```{r decompose, echo=FALSE}
# Compute the decomposition for one observational sample
set.seed(99)
notif_obs <- rbinom(n, 1,
  prob = pmin(1, pmax(0, 0.5 + 0.15 * rct$experience)))
# LPM potential outcomes
y1 <- pmin(1, pmax(0, 0.4 + 0.2 * rct$experience + 0.05))  # under treatment
y0 <- pmin(1, pmax(0, 0.4 + 0.2 * rct$experience))         # under control

naive_est <- mean(y1[notif_obs == 1]) - mean(y0[notif_obs == 0])
att <- mean(y1[notif_obs == 1] - y0[notif_obs == 1])
sel_bias <- mean(y0[notif_obs == 1]) - mean(y0[notif_obs == 0])

tibble(
  Component = c("Naive estimate", "= ATT", "+ Selection bias"),
  Value = c(naive_est, att, sel_bias)
) |>
  knitr::kable(digits = 3)
```
]

.pull-right[
- **ATT** = the real causal effect on treated drivers
- **Selection bias** = treated drivers had higher $Y(0)$ (they would have accepted more even without the notification)
- **Naive** = ATT + bias → overstates the effect
]

---
name: ate-att-atu-matters

# ATE vs ATT vs ATU: When It Matters

Recap — **ATE**: all drivers · **ATT**: opt-in drivers · **ATU**: non-opt-in drivers.

**New scenario:** effects are now *heterogeneous* — novices benefit more than experienced drivers. Average still ~5pp, but ATT and ATU diverge.

.pull-left[
```{r ate-att-dgp}
# Heterogeneous DGP
rct_het <- tibble(
  experience = rnorm(n),
  tau = pmax(0, 0.05 - 0.05 * experience),
  y0  = pmin(1, pmax(0, 0.4 + 0.2 * experience)),
  y1  = pmin(1, pmax(0, y0 + tau)),
  # self-selected (observational)
  D_obs = rbinom(n, 1, prob =
    pmin(1, pmax(0, 0.5 + 0.15 * experience))),
  # randomized
  D_rct = sample(rep(c(0, 1), each = n/2))
)
```
]

.pull-right[
```{r ate-att-out}
rct_het |>
  summarise(
    ATE     = mean(y1 - y0),
    ATT_obs = mean((y1 - y0)[D_obs == 1]),
    ATU_obs = mean((y1 - y0)[D_obs == 0]),
    ATT_rct = mean((y1 - y0)[D_rct == 1]),
    ATU_rct = mean((y1 - y0)[D_rct == 0])
  ) |> round(3)
```
]

---

# Estimand vs Estimator: Simulation ≠ Reality

**Simulation**: we constructed both `y0` and `y1` for every unit, so `mean(y1 − y0)` gives the *true* ATE. **Reality**: for each unit we observe only one outcome, $Y_i = D_i \cdot y1_i + (1 - D_i) \cdot y0_i$. The other is counterfactual.

.pull-left[
```{r estimator-code, eval=FALSE}
rct_het |>
  mutate(
    # observed = realized potential
    Y_obs = if_else(D_obs == 1, y1, y0),
    Y_rct = if_else(D_rct == 1, y1, y0)
  ) |>
  summarise(
    true_ATE  = mean(y1 - y0),
    naive_obs = mean(Y_obs[D_obs == 1]) -
                mean(Y_obs[D_obs == 0]),
    naive_rct = mean(Y_rct[D_rct == 1]) -
                mean(Y_rct[D_rct == 0])
  ) |> round(3)
```
]

.pull-right[
```{r estimator-run, ref.label='estimator-code', echo=FALSE}
```

- `true_ATE` requires both potential outcomes → only in simulation.
- `naive_rct` ≈ `true_ATE` → **RCT identifies ATE** (and ATT, ATU — all equal under randomization).
- `naive_obs` is biased upward — mixes ATT with selection bias.
- RCTs don't recover `ATT_obs` (effect on *actual adopters*) — that's external validity (Module 7).
]

---

# Where Does the Bias Come From?

.pull-left[
Decompose the naive estimator:

`naive_obs` = `ATT_obs` + selection bias

where selection bias $= E[y_0 \mid D{=}1] - E[y_0 \mid D{=}0]$.

```{r bias-decomp}
rct_het |>
  summarise(
    naive_obs = mean(y1[D_obs == 1]) -
                mean(y0[D_obs == 0]),
    ATT_obs   = mean((y1 - y0)[D_obs == 1]),
    sel_bias  = mean(y0[D_obs == 1]) -
                mean(y0[D_obs == 0])
  ) |> round(3)
```
]

.pull-right[
**Two distinct gaps:**

- **ATT_obs vs ATE** (≈ 0.015): driven by *effect heterogeneity* — how much τ varies across who opts in. Modest here.
- **naive_obs vs ATE** (≈ 0.112): driven by *baseline y0 differences* — experienced drivers have higher y0 **and** opt in more.

.highlight-box[
**Key intuition:** selection bias knows nothing about the treatment effect. Even if the notification did *nothing*, the naive comparison would still show a ~0.126 gap — just from who selects in.
]
]

**ATT / ATU coming up:** M5 (**LATE** = compliers' ATT), M6 (subgroup heterogeneity), M7 (ATT_obs via external validity).

---

# SUTVA: A Preview

Everything so far assumed:

1. **No interference:** Alice's notification doesn't affect Bob's accept decision
2. **No hidden versions:** the notification is the same for everyone

--

.highlight-box[
In marketplace experiments, **both assumptions are routinely violated:**
- Notifying many drivers sends them toward the same zone → more supply there, fewer rides per driver; and non-notified drivers face less competition elsewhere
- The "same" notification has different effects depending on time of day, traffic, and driver earnings downstream
]

--

This is Module 2's topic — and it's what tech interviewers care about most. The experimental ideal from this module is the *starting point*, not the ending point.

---

# Key Takeaways

1. **Potential outcomes** define the causal effect: $\tau_i = Y_i(1) - Y_i(0)$. We can never observe both.

2. The naive comparison = **ATT + selection bias**. Selection bias = pre-existing differences between groups.

3. **Randomization** makes treatment independent of potential outcomes → kills selection bias → simple difference in means is unbiased for ATE.

4. Randomization works **in expectation**, not in every sample. Inference handles the sampling variation.

5. **ATE ≠ ATT ≠ ATU** when effects are heterogeneous and treatment is correlated with the effect. Always ask "for whom?"

6. **SUTVA** (no interference, no hidden versions) is assumed throughout — and almost always violated in tech. That's next.

---

# Exercise Preview

In the exercise you will:

1. Simulate potential outcomes for 1,000 drivers with heterogeneous effects
2. Show that the naive comparison is biased (reproduce the decomposition)
3. Show that randomization eliminates bias in expectation (500 simulated RCTs)
4. Compute ATE, ATT, ATU and show when they diverge
5. Break SUTVA in a simple simulation and see what happens to the estimate

See `exercise.R` for the starter code.

---

name: course-map-end

# Course Map

<table>
<tr><th>#</th><th>Module</th><th>Status</th></tr>
<tr><td><b>1</b></td><td><b>The Experimental Ideal</b> <i>(just finished)</i></td><td>✓ done</td></tr>
<tr><td>2</td><td><a href="../module-02/slides.html">SUTVA and When It Breaks</a></td><td>✓ done</td></tr>
<tr><td>3</td><td><a href="../module-03/slides.html">Designing Around Interference</a></td><td>✓ done</td></tr>
<tr><td>4</td><td><a href="../module-04/slides.html">Power and Sample Size</a></td><td>✓ done</td></tr>
<tr><td>5</td><td><a href="../module-05/slides.html">Analyzing Experiments</a></td><td>✓ done</td></tr>
<tr><td>6</td><td><a href="../module-06/slides.html">Multiple Testing & Subgroups</a></td><td>✓ done</td></tr>
<tr><td>7</td><td><a href="../module-07/slides.html">External Validity</a></td><td>✓ done</td></tr>
<tr><td>8</td><td><a href="../module-08/slides.html">Beyond the A/B Test</a></td><td>✓ done</td></tr>
</table>

---
class: center, middle, inverse

# Backup Slides

---
name: selection-proof

# Backup: Deriving the Selection Bias Decomposition

Start with the naive estimand (difference in observed conditional means):

$$E[Y \mid D=1] - E[Y \mid D=0]$$

--

Since $Y = D \cdot Y(1) + (1-D) \cdot Y(0)$, the treated group shows $Y(1)$ and the control group shows $Y(0)$:

$$= E[Y(1) \mid D=1] - E[Y(0) \mid D=0]$$

--

Add and subtract $E[Y(0) \mid D=1]$:

$$= \underbrace{E[Y(1) \mid D=1] - E[Y(0) \mid D=1]}_{\text{ATT}} + \underbrace{E[Y(0) \mid D=1] - E[Y(0) \mid D=0]}_{\text{selection bias}}$$

--

Under randomization, $E[Y(0) \mid D=1] = E[Y(0) \mid D=0]$, so selection bias = 0 and ATT = ATE.

<a href="#selection-decomp" class="nav-btn-br">← back</a>

---
name: ignorability-formal

# Backup: Ignorability — Formal Statement and ATE Identification

.small[
**Setup.** Each unit $i$ has potential outcomes $Y_i(0), Y_i(1)$, observed treatment $D_i \in \{0,1\}$, observed covariates $X_i$, and observed outcome
$$Y_i = D_i Y_i(1) + (1-D_i) Y_i(0) \quad \text{(consistency / SUTVA component).}$$

**Strong ignorability** (Rosenbaum & Rubin, 1983) bundles two conditions:

1. **Conditional independence (CIA):** $\;\big(Y(0), Y(1)\big) \perp\!\!\!\perp D \mid X$
2. **Overlap (positivity):** $\;0 < P(D=1 \mid X = x) < 1\;$ for all $x$ in the support of $X$.
]

--

.small[
**Identification of the ATE.** Under CIA + overlap:
$$
\begin{aligned}
\mathbb{E}[Y \mid D=1, X=x] &= \mathbb{E}[Y(1) \mid D=1, X=x] && \text{(consistency)} \\
&= \mathbb{E}[Y(1) \mid X=x] && \text{(CIA).}
\end{aligned}
$$

Symmetric for $D=0$. Subtracting and integrating over $X$:
$$\text{ATE} \;\equiv\; \mathbb{E}[Y(1) - Y(0)] \;=\; \mathbb{E}_X\!\big[\,\mathbb{E}[Y \mid D=1, X] - \mathbb{E}[Y \mid D=0, X]\,\big].$$

The selection-bias term $\mathbb{E}[Y(0) \mid D=1] - \mathbb{E}[Y(0) \mid D=0]$ collapses *within each $X$-cell*: conditional on $X$, the treated and control groups are exchangeable on $Y(0)$.
]

--

.small[
**What randomization buys you.** Randomization makes $D$ independent of *all* covariates — observed and unobserved — so CIA holds with $X = \emptyset$. We never have to enumerate the confounders. Every other identification strategy (matching, regression, IPW, DiD, IV, RDD, synthetic control) substitutes a weaker, stated assumption for the design-based guarantee. Ignorability is the most common substitute — and the hardest to defend with thin observed covariates.
]

<a href="#ignorability-main" class="nav-btn-br">← back</a>
