...
The carryover slide showed the bias when periods are short. So why not just use very long blocks (one switch per week)?
...
- **Short blocks (small $L$):** many transitions per unit time → carryover bias dominates. Bias scales as $\delta / L$.
- **Long blocks (large $L$):** within-block obs are perfectly correlated in $D$ → fewer effectively independent assignments. Block-clustered variance scales as $L / (CT)$.

**MSE has an interior minimum** at $L^{*} \propto (\delta^{2} CT / \sigma^{2})^{1/3}$. Bojinov, Simchi-Levi & Zhao (2023, *Mgmt Sci*) characterize the MSE-optimal switching design under Markov carryover and give an implementable block-randomized scheme.
]
]

.pull-right[
```{r block-tradeoff, echo=FALSE, fig.height=4.2}
tibble(L = seq(1, 60, length.out = 200)) |>
  ...
  geom_vline(xintercept = (2 * 0.0025 / 0.00012)^(1/3),
             linetype = "dashed", alpha = 0.5) +
  scale_color_manual(values = c("steelblue", "firebrick", "darkorange")) +
  labs(title = "Stylized MSE vs block length",
       subtitle = "Interior minimum where carryover bias balances variance",
       x = "Block length L (periods)", y = "Component", color = "") +
  theme(legend.position = "top")
```
]
...
.small[
**Practical defaults at marketplace scale:** 30-min to 4-hour blocks for matching/pricing; daily for slower outcomes. Pair with a short washout window after each switch and use *block-clustered standard errors* — naive FE SE understates variance.
]
