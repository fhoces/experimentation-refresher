...
|---|---|---|
| **Cluster A/B** (whole cities) | Outcome cycles slowly (days/weeks). Many cities available. | Outcome cycles fast (within hours). Few cities → ICC-driven power loss. |
| **Switchback** (whole-city ON/OFF periods) | Outcome cycles fast — driver positioning + accept rate move within minutes. | Long carryover (driver positioning persists across switches → bias). |
...
**Block length.** Per [M3 §12](../module-03/slides.html#11) (Bojinov MSE-optimal $L^{*} \propto (\delta^2 CT/\sigma^2)^{1/3}$): start with **30-minute blocks**, **5-minute washout** after each switch. Driver positioning roughly resets in 5 min; 30-min blocks balance carryover bias against effective sample.
]
...
- 8 cities × 14 days × 24 h × 2 blocks/h = **5,376 city-blocks**.
...
$$\text{MDE} \approx 2.8 \times \sigma \times \sqrt{\frac{\text{DE}_{\text{eff}}}{n_{\text{eff}}}} \approx 0.4 \text{ pp}$$

where $\text{DE}_{\text{eff}}$ is the design effect from block clustering. **0.4 pp** is well below a plausible prior effect (~3 pp from a pilot).
...
**Standard errors.** Block-clustered SE (per [switchback-formal](#switchback-formal)). Naive HC SE understates variance because within-block obs are perfectly correlated in $D$.

**ITT vs LATE.** This rollout is a server-side flag flipped at the city × time level — no driver-side compliance issue. $\text{ITT} = \text{LATE}$.
...
- **Slow-cycling outcome** (e.g., weekly active drivers): use cluster A/B at the city-week level instead.
