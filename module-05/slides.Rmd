...
drivers <- tibble(
  id = 1:n,
  pre_earnings = rlnorm(n, meanlog = 6.2, sdlog = 0.4),  # ~$500/wk baseline
  notification = rep(c(0, 1), each = n / 2),
  # True effect: +$50/wk. Noise correlated with pre_earnings.
  post_earnings = pre_earnings + 50 * notification +
                  rnorm(n, sd = 80)
)
```
...
```{r dim-estimate}
mean(drivers$post_earnings[drivers$notification == 1]) -
  mean(drivers$post_earnings[drivers$notification == 0])
```

--

This is unbiased for the ATE. But the standard error is large because `post_earnings` is noisy.
...
```{r late}
compliance_rate <- mean(experiment$receives_notif[experiment$assigned == 1])
