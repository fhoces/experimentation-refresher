...
- The randomized signup discount is the instrument: ITT = effect of the discount offer on annual spend; LATE = effect of membership on customers who joined because of the discount (compliers).
- Not all offered customers join (never-takers) and some join regardless (always-takers), so 2SLS recovers the LATE, not the ATE.
- CUPED with pre-period spend as the covariate is the natural variance-reduction strategy: last year's spend strongly predicts this year's and is observed for every customer before the experiment.
- Multiple outcome categories (shipping savings, streaming usage, category spend) require a pre-analysis plan that fixes the primary metric before unblinding to avoid inflating false positives.
- Reporting ITT is the conservative business choice; LATE is the right answer when the question is the causal effect of membership itself on complier spend.
...
---
name: itt-late-examples

# ITT vs LATE in Practice: Ride-Sharing Examples

When does a marketplace company face this decision?

.pull-left[
.small[
**RCT with imperfect compliance** (assigned ≠ exposed; ITT always identified, LATE under monotonicity + exclusion):

- **Driver push notification A/B** — 70% of drivers assigned to "show the zone alert" actually receive it (OS-level disabled, in-app opt-out). The DGP behind slide 13.
- **Promo-code email to riders** — random discount sent; only a fraction open and redeem. ITT = effect of *sending*; LATE = effect on *redeemers*.
- **New app feature behind a flag** — rolled out via app update; older app versions don't have it. ITT respects the rollout; LATE is the per-updater effect.
- **Driver bonus eligibility** — randomized offer of a surge bonus; some drivers don't drive enough to qualify. ITT = effect on the offer pool; LATE = effect on those who hit the threshold.
]
]

.pull-right[
.small[
**IV / encouragement design** (random assignment as instrument for an endogenous behavior):

- **Random promo credit → actual ride-taking** — Cohen, Hahn, Hall, Levitt, Metcalfe (2016, NBER w22627) use Uber surge events as an instrument to estimate consumer surplus.
- **Lottery for early feature access → feature use** — random invite acts as IV for adoption among the invited.
- **Encouragement to switch payment method** — random in-app prompt as IV for the actual switch; downstream effect on retention.
- **Quasi-random algorithmic exposure** — multi-arm A/B with imperfect cross-over: assignment as IV for cumulative time-on-feature.
]
]

.highlight-box[
**Rule of thumb.** Ship decisions → ITT (it's what the rollout will deliver). Mechanism / efficacy questions → LATE. Report both if they diverge meaningfully.
]
...
<tr><td>3</td><td><a href="../module-03/slides.html">Designing Around Interference</a></td><td>✓ done</td></tr>
<tr><td>4</td><td><a href="../module-04/slides.html">Power and Sample Size</a></td><td>✓ done</td></tr>
<tr><td><b>5</b></td><td><b>Analyzing Experiments</b> <i>(just finished)</i></td><td>✓ done</td></tr>
<tr><td>6</td><td><a href="../module-06/slides.html">Multiple Testing & Subgroups</a></td><td>✓ done</td></tr>
<tr><td>7</td><td><a href="../module-07/slides.html">External Validity</a></td><td>✓ done</td></tr>
<tr><td>8</td><td><a href="../module-08/slides.html">Beyond the A/B Test</a></td><td>✓ done</td></tr>
</table>
